<template>
    ...
    <div class="main-container">
        ...
        <div class="upload-page-container">
                ...
                <template #footer>
                    <span class="dialog-footer">
                        ...
                        <el-button class="generate-button" type="primary"
                            style="border-color: rgb(0, 155, 131); background-color: rgb(0, 155, 131); font-size: 1em; color: white;"
            ...
            <div v-else-if="fileName && (tableData.length === 0 || (columns.length === 1 && columns[0].prop === 'placeholder')) && !errorMessage"
                class="empty-state">
...
const makeApiRequest = async (url, method, headers, body = null) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (response.headers.get("content-type")?.includes("application/json")) {
                    ...
                    console.log("API Error Response (JSON):", errorDetails.error.message);
                } else {
        ...
        if (contentType && contentType.includes("application/json")) {
            ...
            return { ok: true, data: { message: `Operation successful (Status: ${response.status})`, data: [], accepted_count: 0, rejected_count: 0 } };
        }
...
const triggerAcceptApiCall = async (dataArray) => {
    ...
    try {
        ...
        if (!data.ok) {
            ...
            apiErrorMessage.value = data.error.details?.message || data.error.message || 'An error occurred.';
            messageBoxTitle.value = 'API Error';
            messageBoxContent.value = apiErrorMessage.value;
            messageBoxType.value = 'error';
            messageBoxPurpose.value = 'apiResponse'; // Or a new purpose like 'apiError'
        ...
        if (error && error.status === 401) {
            ...
            messageBoxType.value = "error";
            apiErrorMessage.value = "An unexpected error occurred.";
        }

        console.error("Error in triggerAcceptApiCall:", error);
        messageBoxType.value = "error";
        messageBoxContent.value = `An unexpected error occurred: ${error.message}`;
        messageBoxPurpose.value = 'apiResponse';
        showMessageBox.value = true;
    } finally {
        dataToSendToApi.value = [];
...
const rejectAllData = async () => {
    ...
    if (mappedObjects.value.length === 0 || !fileName.value) {
        ...
        messageBoxPurpose.value = 'apiResponse';
        showMessageBox.value = true;
        // No isLoading = false here, as it wasn't set to true
        return;
    }
    ...
    // Only append rejectedRows if there are any
    if (rejectedRows.value.length > 0) params.append('rejectedRows', rejectedRowsParam);
